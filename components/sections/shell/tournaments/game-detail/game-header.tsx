...
export function GameHeader({ game, orgSlug }: GameHeaderProps) {
  ...
  const homeColor = game.homeTeam.colors?.[0] ?? DEFAULT_HOME_COLOR;
  const awayColor = game.awayTeam.colors?.[0] ?? DEFAULT_AWAY_COLOR;

  const hasTeamColors = game.homeTeam.colors?.[0] || game.awayTeam.colors?.[0];
    ...
    >
      <div className="mx-auto p-4 md:p-6">
        ...
        <div className="flex items-center justify-center gap-4 md:gap-8 py-4">
          <Link
            href={`/${orgSlug}/teams/${game.homeTeam.slug}`}
          ...
          <Link
            href={`/${orgSlug}/teams/${game.awayTeam.slug}`}
            className="flex flex-col items-center gap-2 flex-1 hover:opacity-80 transition-opacity"
          >
